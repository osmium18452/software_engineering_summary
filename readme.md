...
11. **模块设计时的启发式规则有哪些**
    ...
    - 力争降低模块接口的复杂程度
    - 设计单入口、单出口的模块
    - 模块功能可以预测：相同的输入产生相同的输出。
      反例：使用全局变量或者静态变量会使输出不可预测。
...
12. **什么是变换流，什么是事务流**
    ...
    **事务流：** 参考图5.9，当数据流图具有和图5.9相似的形状时，这种数据流是以事务为中心的，
    ...
    这类数据流应划分为一类特殊的数据流，成为事务流。T则成为事务中心，他完成以下任务：
    1. 接受输入数据（又称为事务）
    2. 分析事务来确定他的类型
    3. 根据事务类型来选取一条数据通路
...
3. **详细设计的工具有哪些,都有哪些优缺点**
   ...
   - 是一种“混杂”语言，它使用一种语言（某种自然 语言）的词汇，同时，却使用另一种语言（某种结构化的程序设计语言）的语法。
...
4. **枚举提高程序局部效率（代码效率）的方法**
    ...
    程序效率：执行速度及占用的内存空间
     ...
     应记住下面三条准则。
     - 效率是一种性能需求，目标值应当在需求分析 阶段给出。软件效率应以需求为准，不应以人力所及为准。
     - 好的设计可以提高效率。
     - 代码效率与代码的简单性相关。
...
4. **什么是白盒测试，什么情况需要进行白盒测试？**
    ...
    白盒法：也叫玻璃盒测试，对软件的过程性细节进行细致的检查，这一方法是将测试对象看作一个打开的盒子，
...
6. *补充*
   ...
   4. 定位的方法
       ...
       - 归纳法：从个别现象推断出一般性结论的思维方法。使用这种方法调试程序时,首先把和错误有关的数据组织起来进行分析,
         以便发现可能的错误原因。然后导出对错误原因的一个或多个假设,并利用已有的数据来证明或排除这些假设。
         当然,如果已有的数据尚不足以证明或排除这些假设,则需设计并执行一些新的测试用例,以获得更多的数据。
       - 试探法：从一般原理或前提出发,经过排除和精化的过程推导出结论。
...
6. **软件配置管理的目标、内容、和对象**
    ...
    软件配置管理（Software Configuration Management，SCM）的主要目标是使软件的变更和修改可以更容易被适应，并减少当变更必须发生时所需花费的工作量。
...
9. **能力成熟度模型（Capability Maturity Model ，CMM）的5个级别**
    ...
    - **初始级：** 软件过程的特征是无序的，甚至是混乱的，项目能否成功完全取决于个人能力。
   - **可重复级：** 软件机构建立了基本的项目管理过程，可追踪成本、进度、功能和质量。
     已经建立起必要的过程规范，对新项目的策划和管理过程是基于以前类似项目的实践经验，使得有类似应用经验的项目能再次取得成功。
   - **已定义级：** 软件机构已经定义了完整的软件过程，软件过程已经文档化和标准化。所有项目组都使用已经文档化的、经过批准的过程来开发和维护软件。
   - **已管理级：** 软件机构对软件过程和软件产品都建立了定量的质量目标，所有重要项目的活动过程都是可度量的。
   - **优化级：** 软件机构集中精力持续不断改进软件过程。这一级的软件机构是一个以防止出现缺陷为目标的机构。
     他有能力识别软件过程中的薄弱环节，并有足够的手段改进他们。
